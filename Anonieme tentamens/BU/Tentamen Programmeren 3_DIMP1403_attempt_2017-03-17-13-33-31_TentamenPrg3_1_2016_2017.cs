...
namespace TentamenPrg3_1_2016_2017
{
    #region Vraag1
    //Vraag 1A
    //Om makkelijk aan de hand van inputvariabelen gegevens te verwerken en te bereiken.
    //

    //Vraag 1B
    //Lijn(lijnNr: int), AddHalte(halte: Halte): void, Halte(naam: string, reisduur: TimeSpan)
    //Bus(busNr: int, lijn: Lijn, vertrektijd: DateTime, voornaam: string, achternaam: string)

    //Vraag 1C
    //Bij een property vraag je gegevens van bijvoorbeeld een instantie variabele op. Ook kun je een property read-only maken een instantie niet.
    //
    #endregion
