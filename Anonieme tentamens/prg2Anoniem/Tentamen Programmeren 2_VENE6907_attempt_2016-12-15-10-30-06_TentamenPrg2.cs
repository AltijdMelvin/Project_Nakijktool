...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        public bool Vraag1(string s1, string s2)
        {
            ...
            // Reverse for loop de array
            for (int i = s1.Length - 1; i >= 0; i--)
            {
                // Aanmaken reverse string
                s1Reverse += s1[i];
            }
            // Checken of het niet null of leeg is
            if (string.IsNullOrEmpty(s1Reverse)) {
                return false;
            }
            // Checken of de reverse string overeenkomt met de 2de string 
            return s1Reverse.Equals(s2);
        }
        ...
        private void Vraag3(double[] xs)
        {
            ...
            {
                double temp = (prev + curr + xs[i + 1]) / 3;
                prev = xs[i];
                curr = xs[i + 1];
                xs[i] = temp;
            }
        ...
        {
            Assert.AreEqual("C#", MeestExamensVoorVak(StudentenDatabase.Students, StudentenDatabase.Courses, StudentenDatabase.Exams));
        }
                                            ...
                                            List<Exam> exams)
        {
            ...
            foreach (Course course in courses)
            {
                ...
                {
                    max = courseMax;
                    nameCourseMax = course.Name;
                }
            }
            return nameCourseMax;
        }
    }
}
