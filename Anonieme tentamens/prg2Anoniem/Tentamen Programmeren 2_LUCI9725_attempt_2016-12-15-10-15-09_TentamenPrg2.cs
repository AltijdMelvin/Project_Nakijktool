...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            char[] xs = s1.ToCharArray();
            char[] ys = s2.ToCharArray();
            Array.Reverse(ys);
        ...
        private bool Vraag2(int[] xs)
        {
            for (int i = 0; i < xs.Length; i++)
            {
                ...
                if (nxt > xs.Length - 1) return false;
                i = nxt;
            }
        ...
        private void Vraag3(double[] xs)
        {
            ...
            for (int i = 1; i < xs.Length - 1; i++)
            {
                ...
                if (i > 1)
                {
                    gem = (xs[i] + old + xs[i + 1]) / 3;
                }
                else
                {
                    gem = (old + xs[i - 1] + xs[i + 1]) / 3;
                }
                old = xs[i];
                xs[i] = gem;
            }
        ...
        public List<int> Vraag4(int k)
        {
            ...
            while (getal < k)
            {
                if (factor == 0) list.Add(1);
                else if (factor == 1) list.Add(1);
                else
                {
                    getal = factor * old;
                    old = getal;
                    if (getal < k) list.Add(getal);
                }
                factor++;
            }          
            return list;
                                            ...
                                            List<Exam> exams)
        {
            string meeste = "";
            int max = 0;
            foreach (Course c in StudentenDatabase.Courses)
            {
                int count = 0;
                
                foreach (Exam e in StudentenDatabase.Exams)
                {
                    if (e.Course == c)
                    {
                        count++;
