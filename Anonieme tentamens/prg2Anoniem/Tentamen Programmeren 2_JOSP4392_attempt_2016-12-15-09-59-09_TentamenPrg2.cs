...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            bool eindantwoord; //initialiseren
            int antwoord;

            antwoord = string.Compare(s1, s2); //string1 naar string2 vergelijken
            if (antwoord == 1) //s1 groter dan s2
            {
                eindantwoord = true;
            }
            else eindantwoord = false; //anders
            return eindantwoord;
        }
        ...
        {
            bool antwoord; //variabele
            foreach (var i in xs) //door de array lopen
            {
                xs[i] = xs[i + 1]; //array index veranderen
                if (i = -1)        //kijken of de array -1 is
                {
                    antwoord = true;   //als de array -1 is, antwoord = true
                    break;              //uit de loop stappen
                }
                else antwoord = false; //als antwoord niet -1 is antwoord = false
            }
            return antwoord;
        }
        ...
        {
            int b;
            int a = k - 1;   //initialiseren
            if (a < k)      //als a kleiner is dan k
            {
                b = a*(a-1)!;    //formule voor faculteit = int b
                a--;            //a--
                List.Add(b);   //aan lijst toevoegen
            }                  
            return List;     //de lijst terughalen
        }
                                            ...
                                            List<Exam> exams)
        {
            ...
            if (Course in Exam == "math")
            {
                math++;
            }
            if (Course in Exam == "coo")
            {
                coo++;
            }
            if (Course in Exam == "se")
            {
                se++;
            }
            else Console.WriteLine("Er is examen gemaakt in een vak dat niet bestaat.");
