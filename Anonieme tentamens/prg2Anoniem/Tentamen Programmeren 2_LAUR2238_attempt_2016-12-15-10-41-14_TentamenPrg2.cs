...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            // eerst maak ik van een string een array van characters
            char[] array = s2.ToCharArray();
            
            // dan loop ik door die array heen en sla ik de waarde van i op.
            // tijdens het lopen verander ik de waarde van array[i] met array[i+1]
            // daarna geef ik array[i+1] de waarde temp wat de opgelsage waarde is van array[i].
            ...
            // dan maak ik weer een string van die array
            string result = new string(array);

            // dan kijk ik of de string s1 gelijk is aan de string result
            // zo ja dan geef ik true terug zo nee false
            if (s1 == result)
            {
                return true;
            }
        ...
        {
            // ik begin bij index 0;
            int idx = 0;

            //dan loop ik door de array heen.
            //de index word in de loop elke keer verandert tot dat ie -1 tegenkomt 
            //dan returnt hij true.
            for (int i= idx; ;)
            {
                idx = xs[idx];
        ...
        public List<int> Vraag4(int k)
        {
            ...
            // maar ik denk dat je een loop moet maken die formulles uitvoert en de uitkomst van die
            // formulle toevoegt aan de lijst. de forloop gaat door tot dat er een getal uit de formulle komt die groter is dan k.
            // als de loop klaar is returnt het de list.
            for ()
            {
                                            ...
                                            List<Exam> exams)
        {
            foreach (Course c in courses)
            {
                ...
                // hier kwam ik niet verder.
                // maar wat ik hier nog moest doen is kijken welk van de vakken de hoogste
                // waarde uiteindelijk heeft want dat betekent dat dat vak zijn naam gereturnd met worden.
                if ()
                {
