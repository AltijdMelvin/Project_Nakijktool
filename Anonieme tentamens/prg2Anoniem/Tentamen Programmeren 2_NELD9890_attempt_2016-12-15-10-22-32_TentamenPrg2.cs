...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            char[] test = s2.ToCharArray(); // create char array from string 's2'
            Array.Reverse(test); // reverse the char array
            if (s1.Equals(new string(test))) return true; // compare the string made out of the reversed char array with 's1'
            else return false;
        }
        ...
        private bool Vraag2(int[] xs)
        {
            ...
            for (int i = 0; i < xs.Length; i++)
            {
                ...
                if (index == -1)
                {
                    return true; // eindig pad gevonden
                }
                else if (index > xs.Length -1)
                {
                    return false; // niet eindig pad
                }
            }
            return false; // oneindige pad
        }
        ...
        public void Test_Vraag5()
        {
            Assert.AreEqual("C#", MeestExamensVoorVak(
                StudentenDatabase.Students, 
                StudentenDatabase.Courses, 
                StudentenDatabase.Exams
                )
            );
        }
        ...
        {
            string[] vakken = { "C#", "Math", "Computer Organisation", "Software Engineering", "Python" };
            int[] counters = { 0, 0, 0, 0, 0 };
            foreach(Exam e in exams)
            {
                switch (e.Course.Name)
                {
                    case "C#":
                        counters[0]++;
                        break;
            ...
            return vakken[Array.IndexOf(counters, max)];
        }
    }
}
