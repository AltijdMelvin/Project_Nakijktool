...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            char[] reversearr = s2.ToCharArray(); //maakt van s2 een char array en draait hem om.
            Array.Reverse(reversearr);
            string reverse = reversearr.ToString(); //zet s2 terug naar een string genaamd reverse.
        ...
        {
            int input0 = Array.IndexOf(xs,0);
            int input1 = Array.IndexOf(xs, 1);
            int input2 = Array.IndexOf(xs, 2);

            int answer1;
            int devanswer1;

            answer1 = input0 + input1 + input2;

            devanswer1 = answer1 / 3; // Dit antwoord stop je in de plaats van de index 1 en dat herhaal je voor de anderen uiteindelijk return je de string weer.
        ...
        {
            List<int> resultlist = new List<int>(); // Nieuwe lijst aanmaken.
            int answer1;
            int answer2;

            answer1 = k - 1;
            answer2 = answer1 * k; // De berekening uitrekenen.

            
            for (int i = 0; i < k; i++) // Alle uitkomsten van de berekeningen in de list zetten.
            {
                resultlist.Insert(i, answer2);
            }
            return resultlist;                                    
        }
