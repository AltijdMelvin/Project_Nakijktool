...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        public bool Vraag1(string s1, string s2)
        {
            ...
            int count = 0;
            int index = 2; //Debugging
            //int index = s2.Length;
            char[] secondStringToArray = s2.ToCharArray();
            for (int x = secondStringToArray.Length; x > 1; x--)
            {
                Reversed[count] = secondStringToArray[index];
                count++;
                index--;
            }
        ...
        private void Vraag3(double[] xs)
        {
            int index = 1;
            double oldValue;
            for (int value = 1; value < xs.Length - 1; value++)
            {
                double one = xs[value - 1];
                double two = xs[value];
                double three = xs[value + 1];
                oldValue = xs[index];  //oldValue veranderd mee als de regel hieronder uitgevoerd wordt. Dit moet uiteraard niet gebeuren.
                xs[value] = (xs[value -1] + oldValue + xs[value +1]) / 3;
            } 
        }
        ...
        public List<int> Vraag4(int k)
        {
            ...
            int multiplyWith = 1;
            int x = 0;
            int toAdd = 0; //Initializing
            if (count == 0)
            {
                result.Add(1);
                count++; //Adds index[0] with an 1 value
            }
            while (result[count -1] < k)
            {
                while (multiplyWith != 0)
                {
                    if (x == 0) //Loop to run just once
                    {
                        toAdd = count * multiplyWith;
                        multiplyWith--;
                        x++;
                    }
                    else
                    {
                        toAdd = toAdd * multiplyWith;
                        multiplyWith--;
                    }
                    //result.Add(count * multiplyWith);
                    //count++; //First time inserts index[1] with 1 * 1 = 1
                }
                if (toAdd > k)
                {
                    return result; //For some reason the list gets emptied if this if statement gets executed... 
                }
                result.Add(toAdd);
                count++;
                multiplyWith = count - 1;
                x = 0;
            }
            return result;
        }
