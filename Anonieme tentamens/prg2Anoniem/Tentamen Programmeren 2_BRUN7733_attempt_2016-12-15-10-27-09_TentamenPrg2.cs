...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        public bool Vraag1(string s1, string s2)
        {
            ...
            int j = 0;
            char[] string_array = new char[s1.Length];
            ...
            string result = "";
            foreach (char s in string_array)
            {
                result += s;
            }
            if (result == s2)
        ...
        private void Vraag3(double[] xs)
        {
            ...
            {
                xs[i] = (backup[i - 1] + backup[i] + backup[i + 1]) / 3.0;
            }
        }
