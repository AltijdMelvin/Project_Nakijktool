...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        private void Vraag3(double[] xs)
        {
            ...
            {
                temp[0] = xs[0];
                temp[xs.Length - 1] = xs[xs.Length - 1];
        ...
        public List<int> Vraag4(int k)
        {
            ...
            {
                int facultystart = i;
                int faculty = facultystart - 1;
                
                while (faculty > 0)
                {
                    facultystart = facultystart * (faculty - 1);
                    faculty -= 1;
                    
                }
                if(facultystart < k && faculty > 0)
                {
                    faculteit.Add(facultystart);
                }               
            }
