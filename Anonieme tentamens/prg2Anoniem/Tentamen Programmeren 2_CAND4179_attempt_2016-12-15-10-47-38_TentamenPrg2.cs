...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            double[] copy = new double[xs.Length]; // kopie van xs zodat je de waardes weer kan gebruiken
            Array.Copy(xs, copy, xs.Length);                                  
            for (int i = 1; i < xs.Length -1; i++)
            {                
                xs[i] = (copy[i -1] + copy[i] + copy[i + 1]) / 3;    
            }
        }
