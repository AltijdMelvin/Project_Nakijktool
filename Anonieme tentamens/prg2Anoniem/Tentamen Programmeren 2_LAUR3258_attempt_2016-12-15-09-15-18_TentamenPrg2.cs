...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            double savedNumber = xs[0];

            for (int i = 1; i < xs.Length - 1; i++)
            {
                double temp = savedNumber;
                savedNumber = xs[i];

                xs[i] = (temp + savedNumber + xs[i + 1]) / 3;
            }
        }
        ...
        public List<int> Vraag4(int k)
        {
            ...
            while(true)
            {
                if (fac.Count * fac[fac.Count - 1] > k)
                    return fac;
                else
                    fac.Add(fac.Count * fac[fac.Count - 1]);
            }
        }
        ...
        public string MeestExamensVoorVak(List<Student> students,
                                            ...
                                            List<Exam> exams)
        {
            int[] cTaken = new int[courses.Count];
            int highest = 0;

            foreach (Exam e in exams)
            {
                cTaken[courses.IndexOf(e.Course)]++;
            }

            for (int i = 0; i < cTaken.Length; i++)
            {
                if (cTaken[i] > cTaken[highest])
                    highest = i;
            }
