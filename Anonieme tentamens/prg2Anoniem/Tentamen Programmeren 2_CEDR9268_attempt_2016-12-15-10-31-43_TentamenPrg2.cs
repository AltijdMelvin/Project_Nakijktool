...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {// reverse de eerste array
         // vergelijk de omgekeerde string met de andere als niet waar dan zijn ze niet gelijk
         // exception voor lege waardes???
            char[] deel1 =  s1.ToCharArray();
            Array.Reverse(deel1);
            string deel2 = new string(deel1);
            if (deel2 == s2)
            {
                return true;
            }
        ...
        {//ipv x++ moet x de waarde in de idex krijgen:x=xs[x]
         // als index n[x]= index x[n] is er een infinite loop
         // als xs[x] is groter dan xs.length return false
        ...
        {//loop : array[x]+array[x+1]+array[x+2] / 3 = array[x(beginnended bij 1)]
            //met x <xs.length
            // er wordt pas vervangen als de bovenstaande loop klaar is
            //array om de getallen bij te houden. 
            double[] nieuw = new double[] { };
            for (int x = 1; x < (xs.Length -1); x++ )
            {//berekenen van waardes en opslaan
                nieuw[x-1] = (xs[x -1] + xs[x] + xs[x + 1]) / 3;

            }
            for (int x = 1; x < xs.Length ; x++)
            {// vervangen van de oude waardes met de nieuwe
                xs[x] = nieuw[x-1];
            }
            return  ;
            //throw new NotImplementedException();
        ...
        {
         //for loop die n toevoegd zolang n kleiner is dan k
            List<int> getallen = new List<int>();
            getallen[0] = 1;
            for (int i = 1; i < k; i++)
            {
                int n = i * (getallen[i-1]);
                if (n >= k) { break; }
                getallen.Add(n);
        ...
        {
            Assert.AreEqual(/*"Wiskunde"*/"C#", MeestExamensVoorVak(
        ...
        public string MeestExamensVoorVak(List<Student> students,
                                            List<Course> courses,
                                            List<Exam> exams)
        {// zoek hoe vaak een vak naam voorkomt en sla het op in variable
            //vergelijk de aantallen met elkaar en return de hoogste
