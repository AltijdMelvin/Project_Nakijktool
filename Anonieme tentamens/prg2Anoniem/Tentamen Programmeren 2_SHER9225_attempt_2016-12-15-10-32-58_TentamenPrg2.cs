...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
           bool same = true;  //Set bool to true
           int index2 = 1; //set second index to 1

           if(s1.Length == s2.Length) //check if the strings have the same lenght
                {
               for(int i = 0; i < s1.Length; i++)
                  {
                     if(s1[i] != s2[s1.Length - index2])
                     {
                        same = false; //if s1's first letter is not equal to s2's first letter starting at the end, set bool the false 
                     }                  
                    index2++;
                  }
                }
           else {
                   same = false; //if the strings don't have the same length, set same to false
                }
           return same;
         }
        ...
        {
            bool notinfinite = false; //set bool
            int i = 0;   //set index
            
            while(i >= 0 && i < xs.Length) //loop untill index is out of array
             {
                i = xs[i];   //set index to the number in the array that is at the index                
             }
            if(i < 0) //if index is negative, set bool to true
                {
                  notinfinite = true;   
                }
            
            return notinfinite; //so if the index is positive, return false
        }
        ...
        {
            int fac = 1; //set fac to 1
            int lastfac = 1; //making some temporary ints for calculations
          
            List<int> Result = new List<int>(); //making a list to return

            for(int i = 1; fac < k; i++) //loop while fac is smaller than k
            {
               Result.Add(fac);         //add fac to list
                                                    
               fac = i * lastfac;       //set fac to new fac calculated with i and lastfac
               lastfac = Result[i -1];       //set lastfac to the saved tempfac
            }
