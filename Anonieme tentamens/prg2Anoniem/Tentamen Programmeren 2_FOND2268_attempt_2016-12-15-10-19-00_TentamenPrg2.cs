...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        private bool Vraag2(int[] xs)
        {
            ...
            {
                if (i + getal > xs.Length) //als de stappen buiten de array lopen, eruit 'schieten'
                {
                    break;
                }
                if (xs[i+getal] == -1) // if (xs[i+stappen] gelijk is aan -1, oneindig pad dus, eruit 'schieten'
                {
                    break;
                }
                volgende = xs[i + getal];
        ...
        private void Vraag3(double[] xs)
        {
            ...
            {
                vorige = Convert.ToInt16(xs[i - 1]);  
                volgende = Convert.ToInt16(xs[i + 1]);
                nieuwewaarde = (vorige + xs[i] + volgende)/ 3;
                    xs[i] = nieuwewaarde;
            }
            //failed maar expected = 33.0d. but was 33.066667d
        ...
        public List<int> Vraag4(int k)
        {
            bool onderk = true;
            int uitkomst = 1;
            List<int> Methode = new List<int>();
            while (onderk == true)
            {
                for (int i = 1; uitkomst <k; i++)
                {
                    uitkomst *= i - 1;
                    if (uitkomst > k)
                    { break; }
                    ...
                    {
                        uitkomst = 1;
                        Methode.Add(uitkomst);
                    }
                    else Methode.Add(uitkomst);
                }
                if (uitkomst >= k)
                {
                    onderk = false;
                                            ...
                                            List<Exam> exams)
        {
            ...
            int aantallessen = 0;
            string examenvak = "";
            foreach(Course les in courses)
            {
                foreach (Exam tentamen in exams)
                {
                    if (tentamen.Course == les)
                    {
                        aantallessen++;
                    }
                }
                if (aantallessen > vorige)
                {
                    vorige = aantallessen;
                    examenvak = les.Name;
                }
