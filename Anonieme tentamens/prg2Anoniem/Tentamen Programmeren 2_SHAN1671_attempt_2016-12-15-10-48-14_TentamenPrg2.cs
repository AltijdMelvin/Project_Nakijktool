...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        // This can also be done by comparing individual characters
        // I.e:  s[i] == s[s.Length-1]
        // But due to my headache, I'm solving it by comparing a reversed string
        ...
        public string MeestExamensVoorVak(List<Student> students,
                                            ...
                                            List<Exam> exams)
        {
            // Everything could be done in using loop, which could be more efficient.
            // But using 2 loops the code is both easier to read and the code
            // doesnt have to compare new and old counters for every exam, so
            // I think using 2 loops is more efficient in this case.

            int[] examCounters = new int[exams.Count];
            foreach (Exam exam in exams)
                examCounters[exam.Course.VakNr - 1]++;

            int maxIndex = 0;
            for (int i = 1; i < examCounters.Length; i++)
                if (examCounters[i] > examCounters[maxIndex])
                    maxIndex = i;

            return courses[maxIndex].Name;
        }
    }
}
