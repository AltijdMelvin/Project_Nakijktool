...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            //als de lengte van de strings niet gelijk zijn kan het nooit true zijn.
            if (s1.Length != s2.Length)
            {
                return false;
            }

            //sla beide strings op in een character array
            List<char> c = new List<char>(s1.ToCharArray());
            List<char> d = new List<char>(s2.ToCharArray());

            //draai de omgedraaide string om
            d.Reverse();

            //loop door de lengte van de array heen
            for(int i = 0; i < c.Count; i++)
            {
                //op het moment dat 1 van de letters niet gelijk is is het altijd false.
                if(c[i] != d[i])
                {
                    return false;
                }
        ...
        {
            int i = 0; //om bij te houden welke index als volgend komt
            int loopCount = 0; //om het aantal loops bij te houden
            while (true)
            {
                //wanneer de loops meer is dan de lengte van de array keer de lengte
                if(loopCount >= xs.Length * xs.Length)
                {
                    return false;
                    break;
                }
                
                i = xs[i]; // de i is het oude nummer
                if(i > xs.Length)
                {
                    return false;
                }
                //als -1 gevonden is is de loop afgelopen
                if(xs[i] == -1)
                {
                    return true;
                }
        ...
        {
            //welke index hetzelfde meot blijven
            int ignoreFirst = 0, ignoreLast = xs.Length-1;

            //het orginele getal dat als laatst aangepast is.
            double lastValue = 0;
            
            
            for(int i = 0; i < xs.Length; i++)
            {
                //het overslaan van het aanpassen van de eerste en laatste maar wel als value zien zodat deze gebruikt word
                if(i == ignoreFirst || i == ignoreLast)
                {
                    lastValue = xs[i];
                    continue;
                }
                ...
                {
                    double t = xs[i]; //opslaan van het orgineel
                    xs[i] = (lastValue + xs[i] + xs[i + 1]) / 3; //veranderen getal
                    lastValue = t; //terug zetten van orgineel voor volgende loop
                }
        ...
        //ik heb geen idee wat faculteiten zijn. maar dit zou het ongeveer zijn
        //als ik wist hoe ik een faculteit uit moest rekenen (snap de formule niet)
        public List<int> Vraag4(int k)
        {
            ...
            while (true)
            {
                int newFaciliteit = c * (c - 1);
                if(k > newFaciliteit)
                {
                    returnList.Add(newFaciliteit);
                    c++;
                    continue;
                }
                else
        ...
        public string MeestExamensVoorVak(List<Student> students,
                                            ...
                                            List<Exam> exams)
        {
            string mostDoneCourseName = "";
            int mostDoneCourseTotal = 0;

            //loop door elke cursus heen
            foreach (Course c in courses)
            {
                int courseCount = 0;
                //loop door elk examen heen
                foreach (Exam e in exams)
                {
                    //als de examen curses gelijk is aan de cursus komt deze dus voor
                    if (e.Course == c)
                    {
                        courseCount++;
                    }
                }
                //als de getelde examens meer zijn dan de vorige (of 0) word dit het meest gemaakte examen.
                if (courseCount > mostDoneCourseTotal)
                {
                    mostDoneCourseTotal = courseCount;
                    mostDoneCourseName = c.Name;
                }
            }
            return mostDoneCourseName;
        }
    }
}
