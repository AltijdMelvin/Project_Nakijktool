    ...
    class TentamenPrg2Tentamen20162017
    {
        ...
        private bool Vraag2(int[] xs)
        {
            ...
            while(xs[index] != -1)
            {
                prevIndex = index;
                index = xs[index];
                if (index >= xs.Length) return false;
                if (prevIndex == xs[index]) return false;
            }
            return true;
        }
        ...
        public List<int> Vraag4(int k)
        {
            ...
            while(faculteit < k)
            {
                if (faculteit != 0) answer.Add(faculteit);
                faculteit = berekenFaculteit(index);
                index++;
            }
            return answer;
        }

        public int berekenFaculteit(int index)
        {
            if (index == 0) return 1;
            return index * berekenFaculteit(index - 1);
        }
        ...
        {
            Assert.AreEqual("Wiskunde", MeestExamensVoorVak(StudentenDatabase.Students, StudentenDatabase.Courses, StudentenDatabase.Exams));
        }

        public string MeestExamensVoorVak(List<Student> students, List<Course> courses, List<Exam> exams)
        {
            IEnumerable<IGrouping<Course, Exam>> groups = exams.GroupBy(exam => exam.Course);
            IGrouping<Course, Exam> biggestGroup = groups.First();
            Course mostTakenCourse = biggestGroup.Key;
            return mostTakenCourse.Name;
        }
    }
}
