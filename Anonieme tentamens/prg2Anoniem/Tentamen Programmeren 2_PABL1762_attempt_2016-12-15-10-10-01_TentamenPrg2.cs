...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        private bool Vraag2(int[] xs)
        {
            ...
            while(!isFinished)
            {
                if ((xs.Length - 1) < Index)
                    return false;
                else if (xs[Index] == -1)
                    return true;
                else if (stepCount == xs.Length)
                    isFinished = true;
                Index = xs[Index];
                stepCount++;
            }
        ...
        {
            double tempStoredPrev = 0;
            double storedPrev = xs[0];
            for(int i = 0; i < xs.Length; i++)
            {
                if (i == 0 || i == (xs.Length - 1))
                    continue;
                tempStoredPrev = xs[i];
                xs[i] = ((storedPrev + xs[i] + xs[(i + 1)]) / 3);
                storedPrev = tempStoredPrev;
            }
        }
        ...
        {
            List<int> faculties = new List<int>();
            int multiplier = 1;
            int currentFaculty = 1;
            while(currentFaculty < k)
            {
                faculties.Add(currentFaculty);
                currentFaculty *= multiplier;
                multiplier++;
            }
            return faculties;
        }
                                            ...
                                            List<Exam> exams)
        {
            ...
            foreach(Course course in courses) 
            {
                ...
                if(count > maxCount)
                {
                    courseMostExams = course.Name;
                    maxCount = count;
                }
                count = 0;
            }
            return courseMostExams;
        }
    }
}
