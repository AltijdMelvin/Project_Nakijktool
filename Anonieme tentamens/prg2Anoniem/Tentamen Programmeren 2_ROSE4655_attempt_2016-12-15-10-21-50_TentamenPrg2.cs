    ...
    class TentamenPrg2Tentamen20162017
    {
        ...
        private bool Vraag2(int[] xs)
        {
            ...
            for (int i = 0; i < xs.Length; i++)
            {
                count++;
                int a = xs[i];
                if (count == xs.Length) return false;
                else if (a < 0) return true;
                else if (a > xs.Length - 1) return false;
                i = a;
            }
            return false;
        }
        ...
        private void Vraag3(double[] xs)
        {
            ...
            for (int i = 0; i < xs.Length; i++)
            {
                double gem = xs[i] = (xs[i] + xs[i + 1] + xs[i - 1]) / 3;
                old = xs[i];
                xs[i] = gem;
        ...
        public List<int> Vraag4(int k)
        {
            ...
            while (getal  < k)
            {
                if (getal == 0) list.Add(1);

                else if (getal == 1) list.Add(1);

                else
                {
                    getal = factor * old;
                    old = getal;
                    if (getal < k) list.Add(factor);
                }
            }
            return list;
        }
