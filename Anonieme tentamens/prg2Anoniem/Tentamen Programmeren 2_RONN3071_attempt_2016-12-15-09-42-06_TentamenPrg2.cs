...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        private bool Vraag2(int[] xs)
        {
            ...
            int index = xs[0];
            while(index != -1 && !(index > xs.Length - 1))
            {
                index = xs[index];
                path.Add(index);
                if (path.Count > xs.Length)
                    return false;
            }
            if (index == -1)
            {
        ...
        private void Vraag3(double[] xs)
        {
            ...
            {
                double current = xs[i];
                xs[i] = (previous + current + xs[i + 1]) / 3;
                previous = current;
            }
        }
        ...
        {
            List<int> facs = new List<int>() { 1 };
            int currFac = 1;
            for(int i = 1; currFac < k; i++)
            {
                currFac = currFac * i;
                if(currFac < k)
                    facs.Add(currFac);
        ...
        public string MeestExamensVoorVak(List<Student> students,
                                            ...
                                            List<Exam> exams)
        {
            //Ik begreep de vraagstelling niet helemaal, maar als de bedoeling
            //was om maar één keer door de exams list de loopen heb ik dat hier 
            //gedaan.
            int[] courseTotals = new int[5] { 0, 0, 0, 0, 0 };
            for(int i = 0; i < exams.Count; i++)
                courseTotals[exams[i].Course.VakNr - 1]++;
            int maxVak = courseTotals[0];
            int maxIndex = 0;
            for(int i = 0; i < courseTotals.Length; i++)
            {
                if(courseTotals[i] > maxVak)
                {
                    maxVak = courseTotals[i];
                    maxIndex = i + 1;
                }
            }
