...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            // check of s1 en s2 zelfde lengte hebben
            if (s1.Length != s2.Length)
            {
                return false;
            }

            //maak een char array om reverse in op te slaan 
            char[] reverse = new char[s1.Length];

            int count = 0;
            int reversecount = s1.Length -1;

            // loop reverse door s1 en wijs toe aan reverse
            foreach(var letter in s1)
            {
                reverse[count] = s1[reversecount];
                count++;
                reversecount--;
            }

            // convert de char array naar een string
            string reversed = new string(reverse);

            //vergelijk s2 met reverse
            if(s2 == reversed)
            {
                return true;
            }
        ...
        {
            //counter om bij te houden waar we zijn         
            int count = 1;

            //loop door de array totdat count xs lengte -1 is
            while (count < xs.Length - 1)
            {
                //bereken nieuwe waarde en pas deze toe
                double nieuweWaarde = (xs[count - 1] + xs[count] + xs[count + 1]) / 3;
                xs[count] = nieuweWaarde;
                count++;
            }
            
            //test error omdat de gegeven 'expected' geen rekening houdt met het feit dat i-1 al berekernd/smooth is  
        }
        ...
        {
            //maak lijst om te returnen
            List<int> result = new List<int>();
            
            // voer formule uit zolang i < k 
            for (int i = 0; i <= k; i++)
            {
                result.Add(k = k * (k - 1));
            }
                                            ...
                                            List<Exam> exams)
        {
            ...
            foreach (var course in courses)
            {
                ...
                foreach (var exam in exams)
                {
                    ...
                    //als vaknaam en examennaam gelijk zijn
                    if (course.Name == exam.Course.Name)
                    {
                        //counter +1
                        counter++;
                        
                        //asl dit vak meer examens heeft dan vorige
                        if(counter > higher)
                        {
                            //sla dit vak op als hoogste
                            higher = counter;
                            vaknaam = course.Name;
                        }
                    }
