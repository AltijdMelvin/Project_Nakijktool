    ...
    class TentamenPrg2Tentamen20162017
    {
        ...
        public bool Vraag1(string s1, string s2)
        {
            ...
            if (s1 != null)
            {
                ...
                {
                    backwards = backwards + s1.Substring(i ,1);
                }
            }
            if (backwards == s2)
            {
        ...
        {
            double remember1 = 0;
            double remember2 = xs[0];
            for (int i = 1; i < xs.Length-1; i++)
            {
                remember1 = remember2;
                remember2 = xs[i];
                xs[i] = (remember1 + xs[i] + xs[i + 1]) / 3; 
            }
                                            ...
                                            List<Exam> exams)
        {
            ...
            {
                int countcourse = 0;
                foreach (Exam test in exams)
                {
                    if (test.Course.Name == course.Name)
                    {
                        countcourse++;
                    }
                }
                if (countcourse > count)
                {
                    count = countcourse;
                    most = course.Name;
                }
