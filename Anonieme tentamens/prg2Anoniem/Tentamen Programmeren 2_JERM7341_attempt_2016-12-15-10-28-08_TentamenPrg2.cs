...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            Assert.AreEqual(true, Vraag1("axp", "pba"));
        ...
        {
            //heeft randall aangepast om 0.1 hoger te krijgen om extra vakken te mogen doen!
        ...
        {
            double[] temp = new double[xs.Length - 1];
            temp[0] = xs[0];
            for (int i = 1; i < xs.Length - 1; i++)
            {
                temp[i] = (xs[i] + xs[i - 1] + xs[i + 1]) / 3;
            }
