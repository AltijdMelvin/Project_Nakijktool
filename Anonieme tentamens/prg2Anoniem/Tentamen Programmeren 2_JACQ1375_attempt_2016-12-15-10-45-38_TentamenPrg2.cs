...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        private void Vraag3(double[] xs)
        {
            ...
            {
                double FE = xs[fep];
                double temperary = ((FE + xs[i] + xs[i + 1]) / 3);
                fep = i + 1;
                xs[i] = temperary;
            }
        ...
        public string MeestExamensVoorVak(List<Student> students,
                                            ...
                                            List<Exam> exams)
        {
            string mostexamsmade = string.Empty;
            ...
            foreach(Course course in courses)
            {
                ...
                foreach (Exam exam in exams)
                {
                    ...
                    if (counter > mostexams)
                    {
                        mostexams = counter;
                        mostexamsmade = course.Name;
                    }
                }
            }
            return mostexamsmade;
        }
    }
}
