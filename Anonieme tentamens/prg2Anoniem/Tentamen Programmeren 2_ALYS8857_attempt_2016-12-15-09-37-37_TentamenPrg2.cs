...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        public bool Vraag1(string s1, string s2)
        {
            ...
            char[] spiegelChar = new char[s1.Length];
            for (int i = 0; i < s1.Length; i++)
            {
                spiegelChar[i] = s1[s1.Length - i - 1];
            }
            
            foreach (char c in spiegelChar)
            {
                result = result + c;
            }
        ...
        private bool Vraag2(int[] xs)
        {
            ...
            for (int i = 1; i <= xs.Length; i++)
            {
                ...
                if (index >= xs.Length)
                {
                    return false; // index niet hoger dan maximaal
                }
            }
            return false; //als de loop vaker gaat dan de lengte van xs[], zit je in een oneindige lus.
        ...
        private void Vraag3(double[] xs)
        {
            ...
            {
                gemiddelde = (saveNum + xs[i] + xs[i + 1]) / 3;
                saveNum = xs[i];
                xs[i] = gemiddelde;
            }
