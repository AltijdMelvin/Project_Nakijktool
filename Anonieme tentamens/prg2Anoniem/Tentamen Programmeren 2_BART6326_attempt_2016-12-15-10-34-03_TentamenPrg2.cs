    ...
    class TentamenPrg2Tentamen20162017
    {
        ...
        public bool Vraag1(string s1, string s2)
        {
            ...
            else
            {
                ...
                {
                    if (s1[i] != s2[s2.Length - 1 - i]) //elke letter van beide strings wordt met elkaar vergeleken, alleen begint s2 op het eind.
                    {
                        return false;
                    }
                }
        ...
        private bool Vraag2(int[] xs)
        {
            ...
            {
                xs[index] = xs[xs[index]];
                if (xs[index] < 0)
                {
                    return true;

                }
                if (xs[index] > xs.Length - 1)
                {
                    return false;
                }
            }
        ...
        private void Vraag3(double[] xs)
        {
            ...
            for (int i = 1; i < xs.Length - 1; i++)
            {
                ...
                temp2 = (temp1 + xs[i] + xs[i + 1]) / 3;
                temp1 = xs[i]; //xs[i] wordt opgeslagen in temp1 zodat er later mee vergeleken kan worden
                xs[i] = temp2;
