...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            double[] VergelijkArray = xs;
            for (int i = 1; i < (xs.Length-1); i++)
            {
                xs[i] = (VergelijkArray[i - 1] + VergelijkArray[i] + VergelijkArray[i + 1]) / 3;
            }
        }
                                            ...
                                            List<Exam> exams)
        {
            ...
            {
                if (count[i] > biggest) { biggest = count[i]; test = i+1; }
            }
            foreach (Exam y in exams)
            {
                if (y.Course.VakNr == test)
                {
                    return y.Course.Name;
                }
            }
            return "Error";
        }
