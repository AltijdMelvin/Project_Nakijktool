...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        public bool Vraag1(string s1, string s2)
        {
            ...
            //for (int i  = woorden.Length - 1; i >= 0; i--)  // reverse the words from S1 
            //{
            //    string T = woorden[i];
            ...
            string[] woorden = { s1 };              // making an array of words on place s1
            Array.Reverse(woorden);                 // reverse words from s1
            if (Convert.ToString(woorden) == s2)      // when woorden = s2 (for example "aap" "paa")
            {
        ...
        private void Vraag3(double[] xs)
        {
            foreach ( int i in xs)
            {
               double nieuwecijfers = xs[i] + xs[i + 1] + xs[i + 2] / 3;
