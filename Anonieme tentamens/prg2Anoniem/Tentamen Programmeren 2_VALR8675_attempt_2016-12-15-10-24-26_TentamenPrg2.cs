...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
        ...
        {
            int reverseIndex = s2.Length - 1;

            // Controleer of de lengte van de strings gelijk is
            if (s1.Length != s2.Length)
                return false;

            for (int i = 0; i < s1.Length; i++)
            {
                if (s1[i] != s2[reverseIndex - i])
                    return false;
            }
        ...
        {
            List<int> faculiteiten = new List<int> { };
            ...
            while (true)
            {
                if (i < 2)
                    faculiteiten.Add(1);
                else if (i * faculiteiten[i - 1] > k)
                    break;
                else
                    faculiteiten.Add(i * faculiteiten[i - 1]);
                i++;
            }
            return faculiteiten;
        }
        ...
        public string MeestExamensVoorVak(List<Student> students,
                                            ...
                                            List<Exam> exams)
        {
            int[] examCount = new int[] { 0, 0, 0, 0, 0 };
            int highest = 0;

            for (int i = 0; i < exams.Count; i++)
            {
                examCount[exams[i].Course.VakNr - 1] += 1;
            }

            for (int j = 1; j < examCount.Length; j++)
            {
                if (examCount[j] < examCount[j - 1])
                    highest = j;
            }
            return exams[highest + 1].Course.Name;
        }
    }
}
