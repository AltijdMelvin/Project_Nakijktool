...
namespace Tentamens20162017
{
    class TentamenPrg2Tentamen20162017
    {
                                            ...
                                            List<Exam> exams)
        {
            ...
            int countSe = 0;
            int countPython = 0;
            foreach (Exam subject in exams)
            {
                if (subject.Course.Name == "C#") countCSharp++;
                else if (subject.Course.Name == "Wiskunde") countMath++;
                else if (subject.Course.Name == "Computer Organisation") countCOO++;
                else if (subject.Course.Name == "Software Engineering") countSe++;
                else if (subject.Course.Name == "Python") countPython++;
            }
            string[] subjects = { "C#", "Wiskunde", "Computer Organisation", "Software Engineering", "Python" };
            int[] counts = { countCSharp, countMath, countCOO, countSe, countPython };
            int maxSubject = counts[0];
            for(int i=1; i<counts.Length; i++)
            {
                if (maxSubject == counts[i])
                {
                    maxSubject = counts[i];
                }else if (counts[i]>maxSubject)
                {
                    maxSubject = counts[i];
                }
            }

            int indexNumber = Array.IndexOf(counts,maxSubject);
            vakNaam=subjects[indexNumber];
            return vakNaam;
        }
    }
}
